...
if (isset($_SESSION["uid"])) {
    ...
    ?>
    <!-- style="border-radius:0.6rem !important;" -->
    <div class="me-5">
        <table class="table table-striped table-primary table-hover mt-4 ms-3">
            ...
            // echo "SELECT * FROM mus_playlists where playlist=" . $_GET['playlist'] . " and user_name=" . $_SESSION["uid"] . " order by add_time desc;";
            if (isset($_GET['playlist'])) {

                $result = mysqli_query(
                    $con
                    ,
                    "SELECT * FROM mus_playlists where playlist='" . $_GET['playlist'] . "' and user_name='" . $_SESSION["uid"] . "' order by add_time desc;"
                );
            } else if (isset($_GET['song_name'])) {
                $result = mysqli_query(
                    $con
                    ,
                    "SELECT * FROM mus_playlists where music_name like '%" . $_GET['song_name'] . "%' and user_name='" . $_SESSION["uid"] . "' order by add_time desc;"
                );
            } else {
                $result = mysqli_query(
                    $con
                    ,
                    "SELECT * FROM mus_playlists where user_name='" . $_SESSION["uid"] . "' order by playlist,add_time desc;"
                );
            }

            while ($row = mysqli_fetch_array($result)) {
                echo "<tr>";
                echo "<td>" . $row['id'] . "</td><td>" . $row['playlist'] . "</td><td>" . $row['music_name'] . "</td><td>"
                    . $row['add_time'] . "</td>";
    ...
    // sleep(2);
    $_SESSION["originalPage"] = "view_music.php";
